...
class Encoder(nn.Module):
    def __init__(self, input_dim, emb_dim, hid_dim, n_layers, kernel_size, dropout, device):
        ...
        self.device = device
        self.scale = torch.qrt(torch.FloatTensor([0.5])).to(device)
        self.tok_embedding = nn.Embedding(input_dim, emb_dim)
        self.pos_embedding = nn.Embedding(3000, emb_dim)
...
class Decoder(nn.Module):
    def __init__(self, output_dim, emb_dim, hid_dim, n_layers, kernel_size, dropout, pad_idx, device):
        ...
        self.pos_embedding = nn.Embedding(200, emb_dim)
        self.emb2hid = nn.Linear(emb_dim, hid_dim)
        self.hid2emb = nn.Linear(hid_dim, emb_dim)
...
print('Loading Data...')
data_train, data_val, data_test = TabularDataset.splits(path='data/', format='json',
                                                             train='cnn_dataset_train.json', validation='cnn_dataset_valid.json', test='cnn_dataset_test.json',
                                                             fields={'story':  ('src', SRC), 'highlights':  ('trg', TRG)})

#data_train, data_val, data_test = Multi30k.splits(exts=('.de', '.en'), fields=(SRC, TRG))
...
train_iterator, valid_iterator, test_iterator = BucketIterator.splits(
    (data_train, data_val, data_test), batch_size = BATCH_SIZE, device = device, sort_key=lambda x: len(x.src), sort_within_batch=False)
...
for epoch in range(N_EPOCHS):
    ...
    if valid_loss < best_valid_loss:
        ...
        torch.save(model.state_dict(), 'model/tut5-model.pt')
...
model.load_state_dict(torch.load('model/tut5-model.pt'))
...
with open("model/trg.Field", "wb")as f:
    dill.dump(TRG, f)
torch.save(model, 'model/tensor.pt')
